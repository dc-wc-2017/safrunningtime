...
import com.successfactors.safrunningtime.visitor.ClassNameVisitor;
...
public class PrintTimeTransformer implements ClassFileTransformer {
	...
	public PrintTimeTransformer(RuleConfiguration config) {
		this.config = config;
	}
    ...
    public byte[] transform(ClassLoader loader, String className, Class<?> classBeingRedefined, ProtectionDomain protectionDomain, byte[] classfileBuffer)
        ...
        reader.accept(new ClassNameVisitor(writer, className, config), 8);
//        reader.accept(new ClassAnnotationVisitor(writer, className), 8);
